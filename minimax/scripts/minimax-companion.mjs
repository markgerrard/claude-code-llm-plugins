...
async function cmdCode(flags, positional) {
  ...
  try {
    ...
    for (let turn = 1; turn <= MAX_TURNS; turn++) {
      console.error(`[minimax:code] Turn ${turn}/${MAX_TURNS}...`);
      ...
      for (const event of events) {
        ...
        if (event.type === "tool_start") {
          turnUsedTools = true;
          console.error(`[minimax:code] Tool: ${event.tool} ${event.args ? JSON.stringify(event.args).slice(0, 80) : ""}`);
        }
      }
      ...
      if (!turnUsedTools) {
        console.error("[minimax:code] No tool calls in this turn — model is done.");
        break;
      }
      ...
      if (lower.includes("all tasks completed") || lower.includes("all 10 tasks") || lower.includes("implementation complete")) {
        console.error("[minimax:code] Model signalled completion.");
        break;
      }
