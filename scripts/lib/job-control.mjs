/**
 * Job querying, filtering, and resolution for Banana jobs.
 */
...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  throw new Error(`No job found for "${reference}". Run /banana:status to list known jobs.`);
}
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Job ${active.id} is still ${active.status}. Check /banana:status and try again once it finishes.`);
  }

  if (reference) throw new Error(`No finished job found for "${reference}".`);
  throw new Error("No finished Banana jobs found for this workspace yet.");
}
...
export function resolveCancelableJob(cwd, reference) {
  ...
  if (activeJobs.length > 1) throw new Error("Multiple Banana jobs are active. Pass a job id to /banana:cancel.");
  throw new Error("No active Banana jobs to cancel.");
}
