...
function pushJobDetails(lines, job, options = {}) {
  ...
  if ((job.status === "queued" || job.status === "running") && options.showCancelHint) {
    lines.push(`  Cancel: /banana:cancel ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && options.showResultHint) {
    lines.push(`  Result: /banana:result ${job.id}`);
  }
...
export function renderStatusReport(report) {
  ...
  if (report.running.length > 0) {
    ...
    for (const job of report.running) {
      const actions = [`/banana:status ${job.id}`, `/banana:cancel ${job.id}`];
      lines.push(
...
export function renderCancelReport(job) {
  const lines = ["# Banana Cancel", "", `Cancelled ${job.id}.`, ""];
  if (job.title) lines.push(`- Title: ${job.title}`);
  if (job.summary) lines.push(`- Summary: ${job.summary}`);
  lines.push("- Check `/banana:status` for the updated queue.");
  return `${lines.join("\n").trimEnd()}\n`;
}
